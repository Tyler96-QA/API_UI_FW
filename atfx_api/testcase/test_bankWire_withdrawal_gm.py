...
LastEditTime: 2023-05-10 11:22:35
...
@allure.suite('国际银行电汇出金')
class TestBankWireWithdrawal(object):

    test_data=HandleExcel(os.path.join(SystemEnv.DATA_DIR,'CP接口用例.xlsx'),'New_BankWire_Withdrawal_Gm').read_excel_data_obj()

    @allure.title("bankwire出金：{case.title}")
    @pytest.mark.high
    @pytest.mark.parametrize('case',test_data)
    @pytest.mark.flaky(reruns=2, reruns_delay=2)
    @pytest.mark.run(order=6)
    def test_bankwire_withdrawal(self,case):
        """
        出金逻辑如下：
        通过接口查询交易账号余额，判断余额是否为0，为0跳过用例
        通过接口查询交易账号是否存在未处理的出金，存在则跳过用例
        接口查询该账号是否存在bankwire出金方式，不存在则跳过用例
        以上条件都满足，出金。BOS转成功。断言出金接口返回数据
        """
        if hasattr(SystemEnv,'balance') and SystemEnv.balance < 1:
            pytest.skip('交易账号余额为0或者余额小于1，跳过用例')
        elif hasattr(SystemEnv,'exist') and SystemEnv.exist == True:
            pytest.skip('交易有未处理的出金请求，跳过用例')
        elif hasattr(SystemEnv,'bankWire') and SystemEnv.bankWire == []:
            pytest.skip('不存在bankwire出金方式，跳过用例')
        else:
            public_request(case)
