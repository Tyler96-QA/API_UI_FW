...
LastEditTime: 2023-05-09 20:04:53
...
渠道币种查询:
atfx-{}_bos_psp_currency 查询语句：{pspId:'neteller'} 查询字段：['currency']
入金汇率查询：
...
@allure.suite('电子钱包入金')
class TestEwalletDeposit(object):

    test_data=HandleExcel(os.path.join(SystemEnv.DATA_DIR,'CP接口用例.xlsx'),'New_Ewallet_Deposit_Gm').read_excel_data_obj()

    @allure.title("dusupay入金：{case.title}")
    @pytest.mark.high
    ...
    def test_dusupay_deposit(self,case):
        ...
        """
        setattr(SystemEnv,'depositType','dusupay')
        public_request(case)


    @allure.title("neteller入金：{case.title}")
    @pytest.mark.high
    @pytest.mark.parametrize('case',test_data)
    @pytest.mark.flaky(reruns=2, reruns_delay=2)
    @pytest.mark.run(order=4)
    def test_neteller_deposit(self,case):
        """
        入金逻辑如下：
        根据主账号查询入金接口所需字段，从数据库获取
        根据交易账号查询交易账号币种，组别，地区等，从数据库获取
        ...
        以上查询为每个接口提供了必传参数。入金只需提供对应测试环境的交易账号，主账号即可
        """
        setattr(SystemEnv,'depositType','neteller')
        public_request(case)

    @allure.title("skrill入金{case.title}")
    @pytest.mark.high
    @pytest.mark.parametrize('case',test_data)
    @pytest.mark.flaky(reruns=2, reruns_delay=2)
    @pytest.mark.run(order=5)
    def test_skrill_deposit(self,case):
        """
        入金逻辑如下：
        根据主账号查询入金接口所需字段，从数据库获取
        根据交易账号查询交易账号币种，组别，地区等，从数据库获取
        ...
        以上查询为每个接口提供了必传参数。入金只需提供对应测试环境的交易账号，主账号即可
        """
        setattr(SystemEnv,'depositType','skrill')
        public_request(case)
